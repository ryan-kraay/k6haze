 ...
 * [yq-go](https://github.com/mikefarah/yq): A lightweight and portable `jq`-like processor for YAML, JSON, and HCL.  We initially used `jq`, but discovered that Talos includes some YAML that is stored as strings, so we needed something like `jq` but for YAML files.  Rather than maintain two tools, we switched to `yq-go` for consistency.
